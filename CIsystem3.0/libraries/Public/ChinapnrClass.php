...
require_once('SecureTool.class.php');
class CI_ChinapnrClass{
	...
	public $confFile = "chinapnr.conf.devel.php";
	...
	const CMDID_ADD_BID_INFO = "AddBidInfo";//标的信息录入
	
	public function __construct()
	{
		$this->logFile = dirname(dirname(dirname(__DIR__))).'/HFlog/hfInterface'.date("Ymd").'.log';

		$npcConfig = include($this->confFile);
		$this->Chinapnr($npcConfig);
	}
	/**
	...
	private function Chinapnr($params){
		...
		$this->merCustId = $params['merCustId'];
	 ...
	 * @return string
	 */
	private function sign($msgData){
		//return $this->scureTool->sign($signData);exit;
		try {
...
HTML;
		$html.='<form id="autoRedirectForm" method="POST" action="'.$this->platformUrl.'">';
	...
	public static function getInstance(){
		if(self::$self == null){
			$params= require_once $this->confFile;
	...
	/**
	 * @desc open an account 标的信息录入 
	 * @link API:5.3.1
	 * 	
	 * @return 无返回，使用autoRedirect方式重定向用户浏览器页面
	 */
	public function objectTypein($merCustId, $proId, $borrCustId, $borrTotAmt, $yearRate, $retType, $bidStartDate, $bidEndDate, $retAmt, $retDate, $proArea, $guarCompId='', $guarAmt='', $bgRetUrl, $merPriv, $reqExt=""){
		...
		$reqData=array(
				...
				"MerCustId"		=>	$merCustId,
				"ProId"			=>	$proId,			# 项目ID
				"BorrCustId"	=>	$borrCustId,	# 借款人ID
				"BorrTotAmt"	=>	$borrTotAmt,	# 借款总金额
				"YearRate"		=>	$yearRate,		# 年利率
				"RetType"		=>	$retType,		# 还款方式
				"BidStartDate"	=>	$bidStartDate,	# 投标开始时间
				"BidEndDate"	=>	$bidEndDate,	# 投标结束时间
				"RetAmt"		=>	$retAmt,		# 应还总金额
				"RetDate"		=>	$retDate,		# 还款日期
				"ProArea"		=>	$proArea,		# 项目所在
				"GuarCompId"	=>	$guarCompId,	# 担保公司ID
				"GuarAmt"		=>	$guarAmt,		# 担保公司金额
				"BgRetUrl"		=>	$bgRetUrl,
				"MerPriv"		=>	$merPriv,
				"ReqExt"		=>  $reqExt,
				"ChkValue"		=>	$checkValue,
		);
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[objectTypein]:".json_encode($reqData);
		$content .= "\r\n[checkValue]:".$this::VERSION_10.$this::CMDID_ADD_BID_INFO.$merCustId.$proId.$borrCustId.$borrTotAmt.$yearRate.$retType.$bidStartDate.$bidEndDate.$retAmt.$retDate.$guarCompId.$guarAmt.$proArea.$bgRetUrl.$merPriv.$reqExt;
		$this->writeLogs($content);
		$res = $this->request($reqData);
		return $res;
	}
	 ...
	 * @return 无返回，使用autoRedirect方式重定向用户浏览器页面
	 */
	public function userRegister($merCustId, $bgRetUrl, $usrMp="", $merPriv="", $retUrl="", $usrId="", $usrName="", $idType="", $idNo="", $usrEmail="", $charSet=""){
		...
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[userRegister]:".json_encode($reqData);
		$content .= "\r\n[checkValue]:".$this::VERSION_10.$this::CMDID_USER_REGISTER.$merCustId.$bgRetUrl.$retUrl.$usrId.$usrName.$idType.$idNo.$usrMp.$usrEmail.$merPriv;
		$this->writeLogs($content);
	...
	public function userLogin($merCustId, $usrCustId)
	{
		...
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[userLogin]:".json_encode($reqData);
		$content .= "\r\n[checkValue]:".$this::VERSION_10.$this::CMDID_USER_LOGIN.$merCustId.$usrCustId;
		$this->writeLogs($content);	
		$this->autoRedirect($reqData);
	}
	/**
	...
	public function acctModify($merCustId, $usrCustId)
	{
		...
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[acctModify]:".json_encode($reqData);
		$content .= "\r\n[checkValue]:".$this::VERSION_10.$this::CMDID_ACCT_MODIFY.$merCustId.$usrCustId;
		$this->writeLogs($content);	
		$this->autoRedirect($reqData);
	}
	/**
	...
	public function autoTenderPlan($merCustId,$usrCustId,$tenderPlanType,$transAmt,$retUrl = '',$merPriv = '')
	{
		...
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[autoTenderPlan]:".json_encode($reqData);
		$content .= "\r\n[checkValue]:".$this::VERSION_10.$this::CMDID_AUTO_TENDER_PLAN.$merCustId.$usrCustId.$tenderPlanType.$transAmt.$retUrl.$merPriv;
		$this->writeLogs($content);
	...
	public function corpRegister($merCustId,$usrId = '',$usrName = '',$instuCode = '',$busiCode,$taxCode = '',$merPriv = '',$charSet = '',$guarType = '',$bgRetUrl,$reqExt = '')
	{
		...
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[corpRegister]:".json_encode($reqData);
		$content .= "\r\n[checkValue]:".$this::VERSION_10.$this::CMDID_CORP_REGISTER.$merCustId.$usrId.$usrName.$instuCode.$busiCode.$taxCode.$merPriv.$guarType.$bgRetUrl.$reqExt;
		$this->writeLogs($content);	
		$this->autoRedirect($reqData);
	}
	/**
	...
	public function autoTenderPlanClose($merCustId,$usrCustId,$retUrl = '',$merPriv = '')
	{
		...
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[autoTenderPlanClose]:".json_encode($reqData);
		$content .= "\r\n[checkValue]:".$this::VERSION_10.$this::CMDID_AUTO_TENDER_PLAN_CLOSE.$merCustId.$usrCustId.$retUrl.$merPriv;
		$this->writeLogs($content);
		$this->request($reqData);
		//$this->autoRedirect($reqData);
	}
	...
	public function netSave($merCustId,$usrCustId,$ordId,$ordDate,$gateBusiId = '',$openBankId = '',$dcFlag = '',$transAmt,$retUrl = '',$bgRetUrl,$merPriv = '')
	{
		...
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[netSave]:".json_encode($reqData);
		$content .= "\r\n[checkValue]:".$this::VERSION_10.$this::CMDID_NET_SAVE.$merCustId.$usrCustId.$ordId.$ordDate.$gateBusiId.$openBankId.$dcFlag.$transAmt.$retUrl.$bgRetUrl.$merPriv;
		$this->writeLogs($content);
		$this->autoRedirect($reqData);
	}
	/**
	 ...
	 * @return 无返回，使用autoRedirect方式重定向用户浏览器页面
	 */
	public function initiativeTender($merCustId,$ordId,$ordDate,$transAmt,$usrCustId,$maxTenderRate,$borrowerDetails,$BorrowerCustId,$BorrowerAmt,$BorrowerRate,$ProId,$IsFreeze,$freezeOrdId = '',$retUrl = '',$bgRetUrl,$merPriv,$reqExt,$pageType = '')
	{

		//$borrowerDetails = $this->arrayToJsonStr($borrowerDetails);
		$reqExt = $this->arrayToJsonStr($reqExt);
		$checkValue= $this->sign($this::VERSION_20.$this::CMDID_INITIATIVE_TENDER.$merCustId.$ordId.$ordDate.$transAmt.$usrCustId.$maxTenderRate.$borrowerDetails.$IsFreeze.$freezeOrdId.$retUrl.$bgRetUrl.$merPriv.$reqExt.$pageType);
		$reqData=array(
		...
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[initiativeTender]:".json_encode($reqData);
		$content .= "\r\n[checkValue]:".$this::VERSION_20.$this::CMDID_INITIATIVE_TENDER.$merCustId.$ordId.$ordDate.$transAmt.$usrCustId.$maxTenderRate.$borrowerDetails.$IsFreeze.$freezeOrdId.$retUrl.$bgRetUrl.$merPriv.$reqExt.$pageType;
		$this->writeLogs($content);	
		$this->autoRedirect($reqData);
	}
	/**
	 ...
	 * @return 无返回，使用autoRedirect方式重定向用户浏览器页面
	 */
	public function tenderCancle($merCustId,$ordId,$ordDate,$transAmt,$usrCustId,$isUnFreeze,$unFreezeOrdId = '',$freezeTrxId = '',$retUrl = '',$bgRetUrl,$merPriv,$reqExt)
	{
		...
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[tenderCancle]:".json_encode($reqData);
		$content .= "\r\n[checkValue]:".$this::VERSION_20.$this::CMDID_TENDER_CANCLE.$merCustId.$ordId.$ordDate.$transAmt.$usrCustId.$isUnFreeze.$unFreezeOrdId.$freezeTrxId.$retUrl.$bgRetUrl.$merPriv.$reqExt;
		$this->writeLogs($content);
		$this->autoRedirect($reqData);
	}
	/**
	...
	public function userBindCard($merCustId,$usrCustId,$bgRetUrl,$merPriv = '')
	{
		...
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[userBindCard]:".json_encode($reqData);
		$content .= "\r\n[checkValue]:".$this::VERSION_10.$this::CMDID_USER_BIND_CARD.$merCustId.$usrCustId.$bgRetUrl.$merPriv;
		$this->writeLogs($content);
		$this->autoRedirect($reqData);
	...
	public function cash($merCustId,$ordId,$usrCustId,$transAmt,$servFee = '',$servFeeAcctId = '',$openAcctId = '',$retUrl = '',$bgRetUrl="",$remark = '',$charSet = '',$merPriv = '',$reqExt = '')
	{
		...
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[cash]:".json_encode($reqData);
		$content .= "\r\n[checkValue]:".$this::VERSION_20.$this::CMDID_CASH.$merCustId.$ordId.$usrCustId.$transAmt.$servFee.$servFeeAcctId.$openAcctId.$retUrl.$bgRetUrl.$remark.$merPriv.$reqExt;
		$this->writeLogs($content);
		$this->autoRedirect($reqData);		
	}	
	/**
	...
	public function usrAcctPay($ordId,$usrCustId,$merCustId,$transAmt,$inAcctId,$inAcctType,$retUrl = '',$bgRetUrl,$merPriv = '')
	{
		...
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[usrAcctPay]:".json_encode($reqData);
		$content .= "\r\n[checkValue]:".$this::VERSION_10.$this::CMDID_USR_ACCT_PAY.$ordId.$usrCustId.$merCustId.$transAmt.$inAcctId.$inAcctType.$retUrl.$bgRetUrl.$merPriv;
		$this->writeLogs($content);
		$this->autoRedirect($reqData);		
	}	
	/**
	...
	public function usrTransfer($merCustId,$ordId,$retUrl,$bgRetUrl,$usrCustId,$inUsrCustId,$transAmt,$merPriv = '',$reqExt)
	{
		...
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[usrTransfer]:".json_encode($reqData);
		$content .= "\r\n[checkValue]:".$this::VERSION_10.$this::CMDID_USR_TRANSFER.$ordId.$usrCustId.$merCustId.$transAmt.$inUsrCustId.$retUrl.$bgRetUrl.$merPriv.$reqExt;
		$this->writeLogs($content);
		$this->autoRedirect($reqData);		
	}
	/**
	...
	public function creditAssign($merCustId,$sellCustId,$creditAmt,$creditDealAmt,$bidDetails,$fee,$divDetails = '',$buyCustId,$ordId,$ordDate,$retUrl = '',$bgRetUrl,$merPriv = '',$reqExt = '')
	{
		...
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[creditAssign]:".json_encode($reqData);
		$content .= "\r\n[checkValue]:".$this::VERSION_10.$this::CMDID_CREDIT_ASSIGN.$merCustId.$sellCustId.$creditAmt.$creditDealAmt.$bidDetails.$fee.$divDetails.$buyCustId.$ordId.$ordDate.$retUrl.$bgRetUrl.$merPriv.$reqExt;
		$this->writeLogs($content);
		$this->autoRedirect($reqData);
	}
	...
	public function fssTrans($merCustId,$usrCustId,$ordId,$ordDate,$retUrl = '',$bgRetUrl,$merPriv = '',$reqExt = '')
	{
		...
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[fssTrans]:".json_encode($reqData);
		$content .= "\r\n[checkValue]:".$this::VERSION_10.$this::CMDID_FSS_TRANS.$merCustId.$usrCustId.$ordId.$ordDate.$retUrl.$bgRetUrl.$merPriv.$reqExt;
		$this->writeLogs($content);
		$this->autoRedirect($reqData);		
	}	
	/**
	...
	public function queryAcctDetails($merCustId,$usrCustId){
		...
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[queryAcctDetails]:".json_encode($reqData);
		$content .= "\r\n[checkValue]:".$this::VERSION_10.$this::CMDID_QUERY_ACCT_DETAILS.$merCustId.$usrCustId;
		$this->writeLogs($content);
		$this->autoRedirect($reqData);		
	}
	/**
	...
	public function bgBindCard($merCustId,$usrCustId,$openAcctId,$openBankId,$openProvId,$openAreaId,$openBranchName = '',$isDefault,$merPriv = '',$charSet = '')
	{
		...
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[bgBindCard]:".json_encode($reqData);
		$content .= "\r\n[checkValue]:".$this::VERSION_10.$this::CMDID_BG_BIND_CARD.$merCustId.$usrCustId.$openAcctId.$openBankId.$openProvId.$openAreaId.$openBranchName.$isDefault.$merPriv;
		$this->writeLogs($content);
	...
	public function delCard($merCustId,$usrCustId,$cardId)
	{
		...
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[delCard]:".json_encode($reqData);
		$content .= "\r\n[checkValue]:".$this::VERSION_10.$this::CMDID_DEL_CARD.$merCustId.$usrCustId.$cardId;
		$this->writeLogs($content);
	...
	public function autoTender($merCustId,$ordId,$ordDate,$transAmt,$usrCustId,$maxTenderRate,$borrowerDetails,$retUrl = '',$bgRetUrl,$merPriv = '')
	{
		...
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[autoTender]:".json_encode($reqData);
		$content .= "\r\n[checkValue]:".$this::VERSION_10.$this::CMDID_AUTO_TENDER.$merCustId.$ordId.$ordDate.$transAmt.$usrCustId.$maxTenderRate.$borrowerDetails.$retUrl.$bgRetUrl.$merPriv;
		$this->writeLogs($content);
	...
	public function loans($merCustId,$ordId,$ordDate,$outCustId,$transAmt,$fee,$subOrdId,$subOrdDate,$inCustId,$divDetails='',$feeObjFlag='',$isDefault='',$isUnFreeze='',$unFreezeOrdId='',$freezeTrxId='',$bgRetUrl,$merPriv='',$reqExt='')
	{
		...
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[loans]:".json_encode($reqData)."\r\n";
		$content .= "\r\n[checkValue]:".$this::VERSION_20.$this::CMDID_LOANS.$merCustId.$ordId.$ordDate.$outCustId.$transAmt.$fee.$subOrdId.$subOrdDate.$inCustId.$divDetails.$feeObjFlag.$isDefault.$isUnFreeze.$unFreezeOrdId.$freezeTrxId.$bgRetUrl.$merPriv.$reqExt."\r\n";
		$this->writeLogs($content);
		...
		// $this->writeLogs("\r\n".date('Y-m-d H:i:s')."\r\n[loans response]:".json_encode($response)."\r\n");
		return $response;
	}
	/**
	...
	public function repayment($merCustId,$ordId,$ordDate,$outCustId,$subOrdId,$subOrdDate,$outAcctId = '',$transAmt,$fee,$inCustId,$inAcctId = '',$divDetails = '',$feeObjFlag,$bgRetUrl,$merPriv = '',$reqExt = '')
	{
		...
		$this->writeLogs("\r\n".date("Y-m-d H:i:s")."\r\n[repayment response]: ".json_encode($response)."\r\n");
		return $response;
	}
	/**
	...
	public function cashAudit($merCustId,$ordId,$usrCustId,$transAmt,$auditFlag,$retUrl = '',$bgRetUrl,$merPriv = '')
	{
		...
		$content = "\r\n".date('Y-m-d H:i:s', time())."\r\n[cashAudit]:".json_encode($reqData);
		$content .= "\r\n[checkValue]:".$this::VERSION_10.$this::CMDID_CASH_AUDIT.$merCustId.$ordId.$usrCustId.$transAmt.$auditFlag.$retUrl.$bgRetUrl.$merPriv;
		$this->writeLogs($content);
