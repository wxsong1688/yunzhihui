...
<div class="index-middle" id="highEndCustom">
    ...
    <div class="index-middle-section show-center">
        <div class="highEndSection1">
            <ul>
                <li class="float-left high-end-current" id="fin_ph" onclick="window.location='/FinanciaTransactions?p=fin_ph'">普惠金融</li>
...
<script type="text/javascript">
    $(".p-btn-go").click(function(){
        ...
        window.location="/FinanciaTransactions?p=fin_ph&pg="+page;
    });
    function investment(id)
    {    
        $.ajax({
            type: "POST", 
            url:'/Project/checkBid',
            data:{pro_id:id},  
            error:function(){  
                alert("error occured!!!");  
            },
            success:function(data){ 
                if(data=='success'){
                    window.location="/Project/bid?pro_id="+id; 
                }else{
                    parent.modalNew('bob',"我要投资",data,'','','','','确定','','/Usercenter','');
                }
            }
        });         
    }
